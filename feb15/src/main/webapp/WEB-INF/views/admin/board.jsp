...
function detail(no){
	...
	window.open('./detail?no='+no, '1타이틀1', 'width=800px, height=800px, scrollbars=yes');
}
                   ...
                   <table class="table table-hover">
					<thead>
						<tr class="row">
							...
							<th class="col-3 text-center">제목</th>
							<th class="col-2 text-center">글쓴이</th>
							<th class="col-1 text-center">날짜</th>
							<th class="col-2 text-center">아이피</th>
					...
					<tbody>
						<c:forEach items="${list }" var="row">
							<tr class="row<c:if test="${row.board_del eq 0}"> bg-dark</c:if>">
								<td class="col-2 text-center">${row.board_no }</td>
								<td class="col-3 " onclick="detail(${row.board_no })">
								${row.board_title }
								</td>
								<td class="col-2 text-center"><a href="./board?searchTitle=3&search=${row.mname }">${row.mname }</a></td>
