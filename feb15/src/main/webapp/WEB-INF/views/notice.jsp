	...
	<section class="page-section" id="services">
		<div class="container">
			<div class="text-center">
				<h2 class="section-heading text-uppercase">(_공지_)</h2>
			</div>
						...
						<table>
							...
							<tbody>
								...
								<tr>
									<td>${row.nno }</td>
									<c:choose>
										<c:when test="${row.checkDate eq sdf }">
											<td onclick="location.href='./noticeDetail?no=${row.nno }'" ><img alt="??" src="./img/new.png" >${row.ntitle }</td>
										</c:when>
										<c:otherwise>
											<td onclick="location.href='./noticeDetail?no=${row.nno }'">${row.ntitle }</td>
										</c:otherwise>
									</c:choose>
									<td>${row.ndate }</td>
									<td>${row.nread }</td>
